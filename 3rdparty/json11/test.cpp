...
/*
 * Enable or disable code which demonstrates the behavior change in Xcode 7 /
 * Clang 3.7, introduced by DR1467 and described here:
 * https://github.com/dropbox/json11/issues/86 Defaults to off since it doesn't
 * appear the standards committee is likely to act on this, so it needs to be
...
/*
 * Beginning of standard source file, which makes use of the customizations
 * above.
 */
#include "json11.hpp"
#include <algorithm>
...
JSON11_TEST_CASE(json11_test) {
  ...
  string err_comment;
  auto json_comment =
      Json::parse(comment_test, err_comment, JsonParse::COMMENTS);
  ...
  json_failing_comment = Json::parse(failing_comment_test, err_failing_comment,
                                     JsonParse::COMMENTS);
  JSON11_TEST_ASSERT(json_failing_comment.is_null());
  JSON11_TEST_ASSERT(!err_failing_comment.empty());
  ...
  JSON11_TEST_ASSERT(
      std::memcmp(uni[0].string_value().data(), utf8, sizeof utf8) == 0);

  // Demonstrates the behavior change in Xcode 7 / Clang 3.7, introduced by
  // DR1467 and described here: https://llvm.org/bugs/show_bug.cgi?id=23812
...

  {
    ...
    struct TestMultiParse {
      ...
      Json expect_parse_res;
        ...
        {" {", 0, 0, {}},
        {good_json, good_json.size(), 1,
         Json(std::map<string, string>{{"k1", "v1"}})},
        {bad_json1, good_json.size() + 1, 1,
         Json(std::map<string, string>{{"k1", "v1"}})},
        {bad_json2, good_json.size(), 1,
         Json(std::map<string, string>{{"k1", "v1"}})},
        {"{}", 2, 1, Json::object{}},
    };
